...
assert args.ppo_steps > 0
assert args.ppo_clip > 0
...
def train(envs, n_envs, actor, critic, actor_optimizer, critic_optimizer, n_steps, discount_factor, ppo_steps, ppo_clip, device):
    """
    ...
    states = torch.zeros(n_steps, n_envs, actor.input_dim).to(device)
    actions = torch.zeros(n_steps, n_envs).to(device)
    ...
    for step in range(n_steps):
        ...
        state = torch.FloatTensor(state) #[n_envs, observation space]
    ...
    next_value = critic(state.to(device)).squeeze(-1)
...
def update_policy(actor, critic, states, actions, log_prob_actions, advantages, returns, actor_optimizer, critic_optimizer, ppo_steps, ppo_clip):
    ...
    #reshape from [n_steps, n_envs, observation space] to [n_steps * n_envs, observation space]
    states = states.view(-1, states.shape[-1])
    actions = actions.view(-1)
    log_prob_actions = log_prob_actions.view(-1)
...
for seed in seeds:
    ...
    for i in tqdm(range((args.max_steps // args.n_steps))):
        ...
        policy_loss, value_loss, entropy = train(train_envs, args.n_envs, actor, critic, actor_optimizer, critic_optimizer, args.n_steps, args.discount_factor, args.ppo_steps, args.ppo_clip, device)
...
np.savetxt(f'results/ns_ppo_{args.n_envs}ne_{args.hidden_dim}hd_{args.n_layers}nl_{args.activation}ac_{args.dropout}do_{args.lr}lr_{args.discount_factor}df_{args.grad_clip}gc_{args.ppo_steps}ps_{args.ppo_clip}pc_{args.n_steps}ns.txt', experiment_rewards, fmt='%d')
