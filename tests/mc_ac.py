...
def train(env, actor, critic, actor_optimizer, critic_optimizer, discount_factor):
    ...
    policy_loss, value_loss = update_policy(actor, critic, returns, log_prob_actions, values, actor_optimizer, critic_optimizer)
...
def update_policy(actor, critic, returns, log_prob_actions, values, actor_optimizer, critic_optimizer):
...
np.savetxt(f'results/mc_ac_{args.hidden_dim}hd_{args.n_layers}nl_{args.activation}ac_{args.dropout}do_{args.lr}lr_{args.discount_factor}df_{args.grad_clip}gc.txt', experiment_rewards, fmt='%d')